...
def in_test() -> bool:
    """Return True if we are currently inside a test"""
    return getattr(sys, "_called_from_test", False)


@dataclass
class CollectedBackgroundTasks:
    """Normallly background tasks run asynchronously. Inside tests, we need to execute them before checking an assertion. This class provides a collection for scheduled background tasks inside tests and a function to execute them synchronously."""
