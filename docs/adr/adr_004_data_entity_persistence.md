...
Project data lives in one database with consistent transactions — no separate infrastructure needed. Table naming conventions prevent collisions. Projects use standard SQLAlchemy for all data access — no extra API to learn. Dependency declaration makes it explicit which workflows access which data.
